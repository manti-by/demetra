...
async def run_ruff_checks(target_path: Path) -> tuple[int, str, str]:
    return await run_command(
        command=[str(UV["path"]), "run", "--active", "ruff", "check", "--quiet"], target_path=target_path
    )


async def run_ruff_fix(target_path: Path) -> tuple[int, str, str]:
    return await run_command(
...
async def run_ruff_check_diff(target_path: Path) -> tuple[int, str, str]:
    return await run_command(
        command=[
            str(UV["path"]),
            "run",
