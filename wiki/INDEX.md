...
- [Test DB isolation and console-only logging](pages/2026-08-18-test-db-isolation-logging.md) — Debug: suite was writing into the live `demetra` DB (non-autouse `setup_test_db`) and the production log (tui's import-time `dictConfig` file handler); made `setup_test_db` autouse + added a `console_only_logging` fixture that strips file handlers and wraps `dictConfig`; 867 tests pass, main DB stays clean, no session/app log files touched. Also fixed console-run session persistence (upsert pending session in `main()`, `PlanError` handling → awaiting input) and websocket log retrieval by `task_id` (no session row required to stream); 869 tests pass (2026-08-18)
