...
- [Worker opencode EACCES on home volume — entrypoint ownership fix](pages/2026-08-19-worker-opencode-home-permissions.md) — Worker plan step on amon-ra failed with `EACCES: permission denied, mkdir '/home/demetra/.local/share/opencode/repos'`: the `demetra_app_data` volume at `/home/demetra` holds root-owned dirs from an older image, so the `demetra` user couldn't create opencode's `~/.local`. Added a root entrypoint that chowns the home volume once per volume (marker-gated, pruning the `.keys` bind mounts plus `opencode/auth.json`) and drops to `demetra` via `setpriv`; `USER demetra` replaced by the entrypoint in the Dockerfile. (2026-08-19)
