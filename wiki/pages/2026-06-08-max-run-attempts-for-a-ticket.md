...
> **Consistency note (2026-09-03, Consistency Agent):** Corrected Step 2 default to `5` (originally `3`) to match current `demetra/settings.py:48`; TL;DR updated to current default.
...
Added `MAX_RUN_ATTEMPTS` (default 5, originally 3 — changed in `8ffc53b`). Before spawning the workflow subprocess the watcher compares the session counter against the max; at the cap it bails instead of running.
