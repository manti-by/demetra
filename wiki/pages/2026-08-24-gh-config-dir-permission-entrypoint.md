...
Redeploy path: `make docker-deploy` rebuilds the image; the per-boot repair fixes the existing `demetra_app_data` volume on next start — no manual `chown`. Safe with concurrent worker replicas (four workers via `make docker-deploy --scale worker=4`; `deploy.replicas` is not in the current compose file): the repair is idempotent.
