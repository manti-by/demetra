...
class TestSessionListingAPI:
    ...
    async def test_list_sessions_returns_sessions(self, authenticated_client: TestClient):
        with patch("demetra.api.tickets.get_sessions", new_callable=AsyncMock) as mock_get_sessions:
            mock_get_sessions.return_value = []

            with patch("demetra.api.tickets.get_current_user", new_callable=AsyncMock) as mock_get_user:
