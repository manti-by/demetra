...
class TestPerformGitMerge:
    ...
    @pytest.mark.asyncio
    async def test_merge_success_and_pushed(self, mock_run_command, mock_git_force_push):
        result = await perform_git_merge(
    ...
    @pytest.mark.asyncio
    async def test_merge_success_nothing_to_push_without_pr_info(
        self,
        mock_run_command,
        mock_git_force_push,
        mock_pr_comment,
    ...
    @pytest.mark.asyncio
    async def test_merge_success_nothing_to_push_with_pr_info(
        self,
        mock_run_command,
        mock_git_force_push,
        mock_pr_comment,
    ...
    @pytest.mark.asyncio
    async def test_merge_success_nothing_to_push_comment_failure(
        self,
        mock_run_command,
        mock_git_force_push,
        mock_pr_comment,
        ...
        # Merge itself still succeeds; comment failure doesn't affect return value
        assert result is True

    @pytest.mark.asyncio
    async def test_merge_conflict_and_resolved(
        self,
        mock_run_command,
        mock_git_force_push,
    ):
        mock_run_command.side_effect = [
            (1, "", "conflict in file.txt"),  # git merge fails
    ...
    @pytest.mark.asyncio
    async def test_merge_conflict_not_resolved(
        self,
        mock_run_command,
        mock_git_force_push,
    ):
